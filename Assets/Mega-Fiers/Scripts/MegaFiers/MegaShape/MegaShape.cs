...
public class MegaShape : MonoBehaviour
{
	...
	Matrix4x4 GetDeformMat(float percent)
	{
		...
		Vector3 ps	= InterpCurve3D(selcurve, alpha, normalizedInterp);
		Vector3 ps1	= InterpCurve3D(selcurve, alpha + 0.001f, normalizedInterp);
	...
	void BuildRibbonMesh()
	{
		...
		for ( int s = 0; s < strands; s++ )
		{
			...
			for ( int i = 0; i <= segments; i++ )
			{
				...
				{
					Vector3 p = tm.MultiplyPoint3x4(cross[v] + soff);
	...
	void BuildBoxMesh()
	{
		...
		for ( int s = 0; s < strands; s++ )
		{
			...
			if ( cap )
			{
				...
				for ( int v = 0; v < 4; v++ )
				{
					Vector3 p = tm.MultiplyPoint3x4(cross[v * 2] + soff);
					tverts[vi] = wtm.MultiplyPoint3x4(p);	//cross[v]);
